...
def download_with_retry(ticker_obj, func_name, **kwargs):
    """Generic wrapper to call yfinance methods with retries and backoff."""
    max_retries = 3
    backoff = 5
    
    for attempt in range(max_retries):
        try:
            func = getattr(ticker_obj, func_name)
            df = func(**kwargs)
...
def download_h1(yahoo_ticker: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        df = download_with_retry(ticker, "history", start=start, interval="1h", auto_adjust=True)
...
def download_d1(yahoo_ticker: str, years: int = 10) -> pd.DataFrame | None:
    ...
    try:
        session = get_yf_session()
        ticker = yf.Ticker(yahoo_ticker, session=session)
        df = download_with_retry(ticker, "history", start=start, interval="1d", auto_adjust=True)
...
def process_pair(pair: str, yahoo_ticker: str, db: DBClient, d1_years: int) -> dict:
    ...
    # Polite delay between H1 and D1 for same ticker
    time.sleep(2.0)
