...
class DualEMAMomentum(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df.loc[(df['fast_ema'] > df['slow_ema']) & (df['adx'] > self.params['adx_min']) & df['is_bull_engulfing'], 'signal'] = 1
        # Bearish: Trend Down + ADX Strong + Bear Engulfing
        df.loc[(df['fast_ema'] < df['slow_ema']) & (df['adx'] > self.params['adx_min']) & df['is_bear_engulfing'], 'signal'] = -1
